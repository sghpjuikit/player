...
public class MpegAudioFileReader extends TAudioFileReader {
    ...
    public AudioFileFormat getAudioFileFormat(InputStream inputStream, long mediaLength) throws UnsupportedAudioFileException, IOException {
        ...
        else if (headStr.contains("m4a")) {
            if (TDebug.TraceAudioFileReader) {
                TDebug.out("Ogg stream found");
            }
            if (weak == null) {
                throw new UnsupportedAudioFileException("M4A AAC stream found");
            }
        } // No, so pushback.
        else {
            pis.unread(head);
     ...
     * Return the AudioInputStream from the given InputStream.
     */
    public AudioInputStream getAudioInputStream(InputStream inputStream, int medialength) throws UnsupportedAudioFileException, IOException {
        if (TDebug.TraceAudioFileReader) {
