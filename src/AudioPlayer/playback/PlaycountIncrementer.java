...
public class PlaycountIncrementer {
    ...
    PlaycountIncrementer() {
        ...
        percIncrementer = new PercentTimeEventHandler(atP, incrementPlayback);
        percIncrementer.setPercMin(pMin);
        percIncrementer.setTimeMin(tMin);
        // initialize time incrementer
        timeIncrementer = new TimeEventHandler(atT, incrementPlayback);
    ...
    public void configureIncrementation() {
        ...
        if (Configuration.increment_playcount == Playcount.IncrStrategy.ON_TIME) {
            removeOld();
            rereadSettings();
            PLAYBACK.realTimeProperty().setOnTimeAt(timeIncrementer);
        } else
        if (Configuration.increment_playcount == Playcount.IncrStrategy.ON_START) {
            removeOld();
            rereadSettings();
            PLAYBACK.addOnPlaybackStart(incrementPlayback);
        } else
        if (Configuration.increment_playcount == Playcount.IncrStrategy.ON_END) { 
            removeOld();
            rereadSettings();
            PLAYBACK.addOnPlaybackEnd(incrementPlayback);
        } else
        if (Configuration.increment_playcount == Playcount.IncrStrategy.NEVER) {
            removeOld();
        }
    }
