...
import GUI.DragUtil;
import GUI.ItemHolders.PlaylistTableSimple;
...
public class PlaylistManagerComponent extends AnchorPane {
    ...
    public void initialize() {        
        ...
        categoryTree.getSelectionModel().getSelectedItems().addListener((Observable o) -> {
            ...
            for(final Playlist p: getAllSelectedPlaylists()) {
     ...
     */
    private void setTable(TitledPane pane, Playlist p) {
     ...
     * TreeItem
     */
    private List<Playlist> getAllSelectedPlaylists() {
     ...
     * @param list 
     */
    private void getAllPlaylists(TreeItem<?> item, List<Playlist> list) {
        if ( item.getValue() instanceof Playlist ) {
            Playlist p = (Playlist)item.getValue();
            if (!list.contains(p)) {
                list.add(p);
            }
        }
