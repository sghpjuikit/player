...
public class Converter extends ClassWidgetController implements SongWriter {
    ...
    public Converter() {
        ...
        // init data
        acts.accumulate(new Act<>("Rename files", File.class, 1, () -> list("Filename"), (file, data) -> {data.forEach((a,b)->System.out.println(a + "   :   " + b));
...
/******************************* helper classes *******************************/
    ...
    private class In extends ValueNode<Tuple2<String,String>> {
...
//                if(!in_out_type_match || !same_size) return;
//                
//                Map<String,List<String>> m = ins.getValues().collect(toMap(in->in._1,in ->findOrDie(gatherTas(),ta->ta.name.get().equalsIgnoreCase(in._2)).getValue()));
//                for(int i=0; i<source.size(); i++)
//                    action.action.accept(source.get(i), mapSlice(m,i));
//            } else {
//                Map<String,String> m = ins.getValues().collect(toMap(in->in._1,in->toS(tas.stream().filter(ta->ta.name.get().equals(in._2)).findFirst().get().getValue(),x->x,"\n")));
