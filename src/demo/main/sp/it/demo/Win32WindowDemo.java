...
/**
 * Demo for capturing Windows DEVICE change events (like USB add/remove)
 * <a href="https://stackoverflow.com/questions/29667565/jna-detect-audio-device-arrival-remove">...</a>
 */
...
public class Win32WindowDemo implements WindowProc {
	...
	public Win32WindowDemo() {
		...
		String windowClass = new WString("MyWindowClass").toString();
	...
	protected void onSessionChange(WPARAM wParam, LPARAM lParam) {
		switch (wParam.intValue()) {
			case Wtsapi32.WTS_CONSOLE_CONNECT -> this.onConsoleConnect(lParam.intValue());
			case Wtsapi32.WTS_CONSOLE_DISCONNECT -> this.onConsoleDisconnect(lParam.intValue());
	...
	protected LRESULT onDeviceChange(WPARAM wParam, LPARAM lParam) {
		switch (wParam.intValue()) {
			case DBT.DBT_DEVICEARRIVAL ->  onDeviceChangeArrival(lParam);
			case DBT.DBT_DEVICEREMOVECOMPLETE ->  onDeviceChangeRemoveComplete(lParam);
			case DBT.DBT_DEVNODES_CHANGED ->  onDeviceChangeNodesChanged(); // lParam is 0 for this wParam
			default -> System.out.println("Message WM_DEVICECHANGE message received, value unhandled.");
		}
		return null;
	}
