...
public class RatingRatingCellFactory implements RatingCellFactory {
	...
	public TableCell<Metadata,Double> apply(TableColumn<Metadata,Double> c) {
		return new TableCell<>() {
			...
			{
				setContentDisplay(GRAPHIC_ONLY); setAlignment(Pos.CENTER); r.icons.bind(APP.maxRating);
				r.partialRating.bind(APP.partialRating); r.updateOnHover.bind(APP.hoverRating);
				r.editable.bind(APP.allowRatingChange); if (c.getUserData().equals(Metadata.Field.RATING))
			...
			protected void updateItem(Double item, boolean empty) {
				super.updateItem(item, empty); if (empty) {
					...
					r.rating.set(item); if (getGraphic()==null) setGraphic(r);
				}
			}
		};
	}
