...
object FieldedPredicateItemNodeCompanion {
   ...
   fun <V, T> predicate(field: ObjectField<V, T>, f: (T) -> Boolean): Predicate<V> =
      when {
         f===IS0 || f===ISNT0 || f===IS || f===ISNT || (f is TypeAwareF<*, *> && f.typeIn.isNullable) -> Predicate { f(field.getOf(it)) }
