...
class PcControls: StackPane() {
   ...
   init {
      ...
      val icons = arrayOf(
         Icon(IconWH.MOON_27).onClickDo { APP.os.sleep().ifErrorDefault() }.withText(LEFT, "Sleep"),
         Icon(IconWH.MOON_THIRD_QUARTER).onClickDo { APP.os.hibernate().ifErrorDefault() }.withText(LEFT, "Hibernate"),
         Icon(IconWH.MOON_14).onClickDo { APP.os.shutdown().ifErrorDefault() }.withText(LEFT, "Shutdown"),
         Icon(IconWH.MOON_0).onClickDo { APP.os.restart().ifErrorDefault() }.withText(LEFT, "Restart"),
         Icon(IconFA.LOCK).onClickDo { APP.os.lock().ifErrorDefault() }.withText(LEFT, "Lock"),
         Icon(IconMD.LOGOUT).onClickDo { APP.os.logOff().ifErrorDefault() }.withText(LEFT, "Log off"),
      )
