...
class SpitListViewSkin<S>(list: ListView<S>): ListViewSkin<S>(list) {
   ...
   fun initScrollAnimationEffect() {
      ...
      skinnable.onEventUp(SCROLL) {
         ...
         if (it.deltaY != 0.0 || it.deltaX != 0.0) {
            ...
            val vBy = -it.deltaY.sign*skinnable.height*scrollAmount/(skinnable.items.size*(skinnable.fixedCellSize.takeIf { it!=Region.USE_COMPUTED_SIZE } ?: 2.em.emScaled))
