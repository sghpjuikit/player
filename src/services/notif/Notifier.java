...
public final class Notifier extends ServiceBase {
	...
	public static void notificationHide() {
	...
	public static void notificationNowPlaying() {
		APP.use(Notifier.class, nm -> nm.songChange(Player.playingItem.get()));
	}

	private static Notification n;
	private static Node songNotificationGui;
	private static SongReader songNotificationInfo;
	...
	public boolean notificationAutohide = false;
	...
	public boolean notificationAnimated = true;
	@IsConfig(name = "Animation duration")
	public Duration notificationFadeTime = millis(500);
	@IsConfig(name = "Position", info = "Position within the virtual bounding box, which is relative to screen or window")
	public PopOver.ScreenPos notificationPos = Screen_Bottom_Right;
	...
	public PopOver.ScreenUse notificationScr = APP_WINDOW;
	...
	public final VarEnum<String> graphics = new VarEnum<String>("Normal",
			...
			v -> {
				if ("Normal".equals(v)) {
					...
					((Pane) songNotificationGui).setPrefSize(-1, -1);
					...
					APP.widgetManager.find(v, NEW, true).ifPresent(wf -> {
						...
						songNotificationInfo = (SongReader) wf.getController();
						((Pane) songNotificationGui).setPrefSize(700, 300);
					});
				}
			});
	...
	public void showNotification(Node content, String title) {
		if (isRunning()) {
			...
			n.setAutoHide(notificationAutohide);
			n.setAnimated(notificationAnimated);
			n.setAnimDuration(notificationFadeTime);
			...
			// show
			n.screen_preference = notificationScr;
			n.show(notificationPos);
		}
	}
