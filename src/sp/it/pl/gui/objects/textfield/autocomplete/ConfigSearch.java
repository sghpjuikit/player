...
import sp.it.pl.gui.objects.textfield.autocomplete.ConfigSearch.Entry;
						...
						getNode().addEventFilter(KeyEvent.KEY_PRESSED, e -> {
							if (!ignoreEvent)
								if (e.isControlDown() && (e.getCode()==KeyCode.UP || e.getCode()==KeyCode.DOWN)) {
									...
									if (e.isControlDown()) textField.selectPreviousWord();
									else textField.selectBackward();
									if (!e.isShiftDown()) textField.deselect();
									e.consume();
								} else if (e.getCode()==KeyCode.RIGHT) {
									if (e.isControlDown()) textField.selectNextWord();
									else textField.selectForward();
									if (!e.isShiftDown()) textField.deselect();
									e.consume();
								} // else if (!e.getCode().isNavigationKey()) {}
							ignoreEvent = false;
