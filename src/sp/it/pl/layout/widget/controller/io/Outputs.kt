...
class Outputs {
    ...
    fun <T> finOutput(type: Class<T>, name: String): Output<T>? {
        val i = m[name]
        if (i!=null && type.isSuperclassOf(i.type)) throw ClassCastException()
        return i as Output<T>
    }

    fun <T> getOutput(type: Class<T>, name: String): Output<T> = finOutput(type, name)!!

    inline fun <reified T> findOutput(name: String) = finOutput(T::class.java, name)

    inline fun <reified T> getOutput(name: String) = finOutput(T::class.java, name)!!
