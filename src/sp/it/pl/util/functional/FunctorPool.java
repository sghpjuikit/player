...
import sp.it.pl.util.collections.map.PrefListMap;
...
import sp.it.pl.util.functional.Functors.Parameter;
import sp.it.pl.util.functional.Functors.PƑ;
import sp.it.pl.util.functional.Functors.PƑ0;
import sp.it.pl.util.functional.Functors.PƑ1;
import sp.it.pl.util.functional.Functors.PƑ2;
import sp.it.pl.util.functional.Functors.PƑ3;
import sp.it.pl.util.functional.Functors.Ƒ1;
import sp.it.pl.util.functional.Functors.Ƒ2;
import sp.it.pl.util.functional.Functors.Ƒ3;
import sp.it.pl.util.functional.Functors.Ƒ4;
...
import static sp.it.pl.util.Util.StringDirection;
import static sp.it.pl.util.Util.StringDirection.FROM_START;
import static sp.it.pl.util.file.AudioFileFormat.Use.APP;
import static sp.it.pl.util.file.AudioFileFormat.Use.PLAYBACK;
...
import static sp.it.pl.util.text.UtilKt.isPalindrome;
import static sp.it.pl.util.type.Util.getEnumConstants;
import static sp.it.pl.util.type.Util.getSuperClassesInc;
...
public class FunctorPool {
...

	{
		...
		add("-> file name",  S, S, sp.it.pl.util.Util::filenamizeString);
		add("Anime",         S, S, sp.it.pl.util.Util::renameAnime);
		add("To upper case",       S, S, String::toUpperCase);
		add("To lower case",       S, S, String::toLowerCase);
		add("Plural",              S, S, English::plural);
		add("Replace 1st (regex)", S, S, sp.it.pl.util.Util::replace1st, Pattern.class, S, Pattern.compile(""), "");
		add("Remove 1st (regex)",  S, S, sp.it.pl.util.Util::remove1st, Pattern.class, Pattern.compile(""));
		add("Replace all",         S, S, sp.it.pl.util.Util::replaceAll, S, S, "", "");
		add("Replace all (regex)", S, S, sp.it.pl.util.Util::replaceAllRegex, Pattern.class, S, Pattern.compile(""), "");
		add("Remove all",          S, S, sp.it.pl.util.Util::removeAll, S, "");
		add("Remove all (regex)",  S, S, sp.it.pl.util.Util::removeAllRegex, Pattern.class, Pattern.compile(""));
		add("Text",                S, S, (s, r) -> r, S, "");
		add("Re-encode",           S, S, (s, c1, c2) -> new String(s.getBytes(c1), c2), Charset.class, Charset.class, UTF_8, UTF_8);
		add("Add text",            S, S, sp.it.pl.util.Util::addText, S, StringDirection.class, "", FROM_START);
		...
		add("Starts with",   S, B, sp.it.pl.util.Util::startsWithNoCase, S, B, "", true);
		...
		add("Char at",       S, Character.class, sp.it.pl.util.Util::charAt, Integer.class, StringDirection.class, 0, FROM_START);
		...
		add("Anime",        File.class,S, f -> sp.it.pl.util.Util.renameAnime(UtilKt.getNameWithoutExtensionOrRoot(f)));
