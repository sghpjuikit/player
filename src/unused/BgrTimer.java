 ...
 * once per time interval by utilizing background threads.
 * <p>
 * This class is intended for use for tasks running on bgr threads as opposed to
 * applicationFX thread, which is aimed for with solutions such as
 * {@link Timeline}.
 */
public final class BgrTimer {
    /**
     ...
     */
    public BgrTimer(String name, Duration period, Procedure task) {
        setName(name);
        setPeriod(period);
        setTask(task);
    }
     ...
     */
    public BgrTimer(String name, Duration period, Procedure task, boolean useFXThread) {
        setName(name);
        setPeriod(period);
        setTask(task);
    ...
    public void start() {
        if (isRunning()) return;
        if (useFXThread)
            thread = new Thread(() -> loopTaskOnFXThread() );
        else
            thread = new Thread(() -> loopTask() ); 
