...
public interface Util {
	...
	static void throwIf(boolean v) {
		if (v) throw new IllegalStateException("Requirement condition not met");
	}

	static void throwIf(boolean v, String s) {
		if (v) throw new IllegalStateException("Requirement condition not met: " + s);
	}

	static void throwIfNot(boolean v) {
		if (!v) throw new IllegalStateException("Requirement condition not met");
	}

	static void throwIfNot(boolean v, String s) {
		if (!v) throw new IllegalStateException("Requirement condition not met: " + s);
	}
	...
	static void throwIfFinal(Field f) {
		if (Modifier.isFinal(f.getModifiers()))
			throw new IllegalStateException("Final field forbidden. Field=" + f.getDeclaringClass() + "." + f.getName());
	}

	static void throwIfNotFinal(Field f) {
		if (!Modifier.isFinal(f.getModifiers()))
	...
	/**
	 * Returns all running threads. Incurs performance penalty, do not use besides debugging purposes.
	 */
