...
open class V<T>(value: T): SimpleObjectProperty<T>(value) {
   ...
   // helps Kotlin with null-safety inference
   override fun <U: Any?> map(mapper: Function<in @NotNull T, out U>): ObservableValue<U> = super.map(mapper)

   // helps Kotlin with null-safety inference
   override fun <U: Any?> flatMap(mapper: Function<in @NotNull T, out ObservableValue<out U>>): ObservableValue<U> = super.flatMap(mapper)
