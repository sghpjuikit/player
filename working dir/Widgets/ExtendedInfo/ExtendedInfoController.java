 ...
 * @author uranium
 */
public class ExtendedInfoController extends WidgetController {
    ...
    public void initialize() {
        ...
        columnKey.setCellFactory(new Callback<TableColumn<CommentExtended, String>, TableCell<CommentExtended, String>>() {
            @Override
            public TableCell<CommentExtended, String> call(TableColumn<CommentExtended, String> p) {
...
//                .subtract(5)); //(key column) + 5 (additional space to eliminate scrollbar
        columnIValue.setCellFactory(new Callback<TableColumn<CommentExtended, String>, TableCell<CommentExtended, String>>() {
            @Override
            public TableCell<CommentExtended, String> call(TableColumn<CommentExtended, String> p) {
                TableCell<CommentExtended, String> cell = new TableCell() {
                    @Override
        ...
        columnPlaying.setCellFactory(new Callback<TableColumn<Chapter, String>, TableCell<Chapter, String>>() {
            @Override
            public TableCell<Chapter, String> call(TableColumn<Chapter, String> p) {
        ...
        columnTime.setCellFactory(new Callback<TableColumn<Chapter, Duration>, TableCell<Chapter, Duration>>() {
            @Override
            public TableCell<Chapter, Duration> call(TableColumn<Chapter, Duration> p) {
                TableCell<Chapter, Duration>  cell = new TableCell() {
                    @Override protected void updateItem(Object item, boolean empty) {
                        super.updateItem(item, empty);
                        if (empty) setText("");
                        else setText(Util.formatDuration((Duration)item));
                    }
                };
                cell.setAlignment(Pos.CENTER_RIGHT);
                return cell;              
        ...
        columnCValue.setCellFactory(new Callback<TableColumn<Chapter, String>, TableCell<Chapter, String>>() {
            @Override
            public TableCell<Chapter, String> call(TableColumn<Chapter, String> p) {
...
//                                                    .subtract(30)); // 30 = bottom anchor set up in .fxml
        ...
        PlaylistManager.selectedItemProperty().addListener((ObservableValue<? extends PlaylistItem> ov, PlaylistItem t, PlaylistItem t1) -> {
        ...
        PLAYBACK.currentTimeProperty().addListener((Observable o) -> {
            if (source == null ||
                    !source.same(PlaylistManager.getPlayingItem())) { return; }
    ...
    private void getSource() {
        ...
        Task<Metadata> t = MetadataReader.create(item, new OnEnd<Metadata>() {
            @Override public void success(Metadata result) {
                new_meta = result;
                populateGUI();
            }
...
/******************************************************************************/
    ...
    public void playChapter(int index) {
        ...
        if (PlaylistManager.getPlayingItem().same(source)) {
            ...
            PlaylistManager.addUri(source.getFile().toURI());
