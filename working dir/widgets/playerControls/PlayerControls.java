...
public class PlayerControls extends FXMLController implements PlaybackFeature {
	...
	public final V<Seek> seekType = new V<>(Seek.RELATIVE);
	...
	Icon f1    = new GlowIcon(ANGLE_DOUBLE_LEFT,25).onClick(() -> Player.seekBackward(seekType.get())),
		 f2    = new GlowIcon(FAST_BACKWARD,25).onClick(PlaylistManager::playPreviousItem),
		 f3    = new GlowIcon(PLAY,25).onClick(Player::pause_resume),
		 f4    = new GlowIcon(FAST_FORWARD,25).onClick(PlaylistManager::playNextItem),
		 f5    = new GlowIcon(ANGLE_DOUBLE_RIGHT,25).onClick(() -> Player.seekForward(seekType.get())),
		 f6    = new GlowIcon(STOP,25).onClick(Player::stop),
		 muteB = new GlowIcon(VOLUME_UP,15).onClick(Player::toggleMute),
		 loopB = new GlowIcon(RANDOM,15).onClick((MouseEvent e) -> Player.toggleLoopMode(e));
    ...
    private void timeChanged() {
        ...
        if (lastUpdatedTime+1000 <= millis) {
            ...
            realTime.setText(formatDuration(Player.player.realTime.get()));
        }
    }
